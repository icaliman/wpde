	...
	if ( currentLogs[ name ] ) {
		spinnies.succeed( name, {
			text: [
				...
				' after ',
				chalk.red(
					prettyHrtime( process.hrtime( currentLogs[ name ] ) )
				),
			].join( '' ),
		} );
...
module.exports = function ( tasks = [], config ) {
		...
		return ( done ) => {
			...
			if ( dynamicTasks.length ) {
				gulp.series(
					...
					gulp[ isParallel ? 'parallel' : 'series' ](
						...dynamicTasks
					),
					( cb ) => {
						if ( name ) {
	...
	gulp.task(
		'copy',
		runStream( 'copy', allTasks.copy.fn( isDev, browserSync ) )
	);
	...
	gulp.task(
		...
		runStream(
			'remote_copy',
			allTasks.remote_copy.fn( isDev, browserSync )
		)
	);
	...
	gulp.task(
		...
		runStream(
			'prefix_scss',
			allTasks.prefix_scss.fn( isDev, browserSync )
		)
	);
	...
	gulp.task(
		...
		runStream(
			'compile_scss',
			allTasks.compile_scss.fn( isDev, browserSync )
		)
	);
	...
	gulp.task(
		...
		runStream(
			'compile_jsx',
			allTasks.compile_jsx.fn( isDev, browserSync )
		)
	);
	...
	gulp.task(
		'zip',
		runStream( 'zip', allTasks.zip.fn( isDev, browserSync ) )
	);
